...
class Camera:
    ...
    @classmethod
    def get_reference_camera_ezmocap(cls, cam_params, orthographic=False, device='cpu'):
        K = np.identity(3)
        K[0, 0] = cam_params['fx']
        K[1, 1] = cam_params['fy']
        K[0, 2] = cam_params['height'] // 2
        ...
        t = [0, 0, cam_params['cam_center'][2]]
        camera = Camera(K, R, t, orthographic, device=device)
        return camera.P.detach().cpu().numpy()[None, ...]

    @classmethod
    def perspective_camera_with_angle(cls, pitch=0, yaw=0,
                          orthographic=False, cam_params=None,
                          device='cpu'):

        p = np.deg2rad(int(pitch))
        R_pitch = np.array([[1, 0, 0],
                      [0, np.cos(p), -np.sin(p)],
                      [0, np.sin(p), np.cos(p)]])
        y = np.deg2rad(int(yaw))
    ...
    @classmethod
    def perspective_camera_with_angle_recon(cls, view_angle=0, pitch=0,
                          orthographic=False, cam_params=None,
                          device='cpu'):
