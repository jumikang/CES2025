...
class Mesh:
    ...
    def __init__(self, vertices, indices, colors=None, device='cpu', grad_true=False):
        self.device = device

        if grad_true:  # vertices can be updated w.r.t. the smpl parameters
            self.vertices = vertices
        else:
            self.vertices = vertices.to(device, dtype=torch.float32) if torch.is_tensor(vertices) else torch.tensor(vertices, dtype=torch.float32, device=device) if vertices is not None else None
        ...
        if self.indices is not None and self.vertices is not None:
    ...
    def compute_normals(self, canonical=False):
        # Compute the face normals
        if canonical:
            a = self.v_posed[self.indices][:, 0, :]
            b = self.v_posed[self.indices][:, 1, :]
            c = self.v_posed[self.indices][:, 2, :]
            vertex_normals = torch.zeros_like(self.v_posed)
        else:
