...
class SMPLMesh(TexturedMesh):
    ...
    def set_canonical_smpl_in_uv(self, smpl_params):
        ...
        if self.lbs_weights is None:
            lbs_root = os.path.join(self.smpl_config["smpl_root"], self.smpl_config["lbs_weights"])
            with open(lbs_root, 'rb') as f:
    ...
    def forward_skinning(self, smpl_params=None, v_posed=None, update_smpl=False):
        ...
        v_posed_homo = torch.cat([v_posed.detach().cpu(), homogen_coord], dim=2)
        v_homo = torch.matmul(T.to(self.device), torch.unsqueeze(v_posed_homo.to(self.device), dim=-1))
